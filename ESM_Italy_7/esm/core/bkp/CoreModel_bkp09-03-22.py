...
class Base(Identifiers):
    ...
    def _model_generation(self,
        ...
        for item in self.exogenous:
            ...
            if item in ['xy_mix','xy_mix_min','xy_mix_max','cap_o_min','cap_o_max','cap_n_min','cap_n_max','E','E_tld','E_tld_diag','e','mr','te']:
        ...
        for y in self.ids.all_years:
            ...
            if y == self.ids.warm_up_years[0]:
                ...
                esm_eqs.append( p('cap_o',y) == p('cap_o',y-1) + p('cap_n',y) - p('cap_d',y) )
            ...
            for flow in self.ids.emission_by_flows:
                ...
                esm_eqs.append( p('BV_E',y,lv=flow) == cm.multiply(p('E',y), cm.trsp(p('bv',y,r1=flow))) )
            ...
            for item in list(_COST_ITEMS.keys()) + self.ids.emission_by_flows + self.ids.emission_by_techs:
                ...
                # costs related to emissions (by technology)
                    ...
                    # investment cost
                    esm_eqs.append( p('CU',y,r1='c_in',c1='allr',c2='tc') == cm.multiply(p('cu',y,r1='c_in',c1='allr',c2='tc'), p('cap_n',y)) )

                    # decommissioning cost
                    esm_eqs.append( p('CU',y,r1='c_ds',c1='allr',c2='tc') == cm.multiply(p('cu',y,r1='c_ds',c1='allr',c2='tc'), p('cap_d',y)) )
    ...
    def _model_completion(self):
        ...
        def weib(reg  : str,
            ...
            for y,cluster in self._year_cluster('all'):
                ...
                weib_distr = (shape / tech_life) * (years / tech_life)**(shape - 1) * np.exp(-(years / tech_life)**shape)
        ...
        for reg in self.Regions:
            ...
            d_rate = np.array([(1/(1+self.data['mr'].loc[:,(reg,'mr_d')])**(np.arange(len(self.ids.all_years)))).values]).T

            # fuel, variable, disposal, other costs: discounting money
            for cost_item in list(_COST_ITEMS.keys()) + self.ids.emission_by_flows + self.ids.emission_by_techs:
    ...
    def _data_assigment(self):
        ...
        for item in self.exogenous:
            ...
            elif item in ['xy_mix','xy_mix_min','xy_mix_max','cap_o_max','cap_o_min','cap_n_max','cap_n_min','te']:
    ...
    def _reallocate_blender_emission(self, region):
        ...
        for yy,clst in self._year_cluster('all'):
            ...
            gas_content = (xy.loc[:,(reg,'t.gas_blender')]/xy.loc[:,(reg,techs)].sum(1)).values[0]

            emission_factor = self.data['bv'][clst['bv']].loc['f.eCO2',(slice(None),'f.natgas')].values[0] * gas_content

            u = self.data['u'][clst['u']].loc[(reg,'f.blended_gas'),:].to_frame().T
            U = pd.DataFrame(u.values*xy.values,index=u.index,columns=u.columns)

            all_techs = df_U.columns.get_level_values(-1)
